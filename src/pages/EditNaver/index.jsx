...
import { editNaver } from '../../api/services/serviceEdit';
...
export default function EditNaver({ match }) {
  ...
  useEffect(() => {
    async function getNaver() {
      const user = await editNaver('GET', id);

      if (user !== 404) {
        setName(user.name);
        setBirthDate(calculateTime(user.birthdate));
        setProject(user.project);
        setJobRole(user.job_role);
        setAdmissionDate(calculateTime(user.admission_date));
        setUrl(user.url);
        setFeedBackTitle('Naver atualizao');
        setFeedText('Naver atualizado com sucesso!');
      } else if (user === 404) {
        setFeedBackTitle('Erro');
        setFeedText('Usuário não encontrado');
      } else {
        setFeedBackTitle('Erro');
        setFeedText('Não foi possível editar o usuário');
      }
    }
    getNaver();
  }, [id]);
  ...
  function submitEdit(event) {
    ...
    editNaver('PUT', id, jobRole, admission, age, name, project, url);
    setFeedBack(true);
    return;
  }
